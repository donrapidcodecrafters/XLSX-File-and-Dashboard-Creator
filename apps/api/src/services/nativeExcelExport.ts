/**
 * Server-side port of apps/web/src/lib/nativeExcelDashboardExport.ts.
 *
 * Builds workbooks with genuine native Excel chart objects (xl/charts/*.xml,
 * fully editable in Excel) instead of a flattened chart image — the same
 * technique the Studio app's own "Export Workbook" button uses client-side.
 * Used for every server-generated workbook (background export jobs and
 * scheduled/test emails) so they match what a user gets from Studio itself,
 * rather than drifting via a separate image-rendering implementation.
 *
 * Keep in sync with the browser version — this is a deliberate duplicate
 * (Blob/ArrayBuffer swapped for Buffer) rather than a shared import, since
 * apps/web dynamic-imports exceljs/jszip for bundle-size reasons and apps/api
...
function writeRowsSheet(sheet: any, report: ReportDefinition, table: TableDefinition | undefined, result: ReportRunResult, startRow: number, startCol: number) {
...
async function getWorkbookSheetPaths(zip: JSZip) {
...
async function injectNativeCharts(workbookBuffer: Buffer, chartsBySheet: Map<string, NativeChartPlacement[]>): Promise<Buffer> {
  ...
  return zip.generateAsync({ type: "nodebuffer", mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
}
...
export async function exportDashboardNativeChartWorkbook(
  ...
  rendered.tabs.forEach((tab) => {
    ...
    if (overview) overviewRow += 1;
    chartsBySheet.set(sheet.name, sheetCharts);
  });

  const buffer = Buffer.from(await workbook.xlsx.writeBuffer());
  return injectNativeCharts(buffer, chartsBySheet);
}
