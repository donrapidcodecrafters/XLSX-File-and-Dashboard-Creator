...
function isAutoGeneratedLabel(label: string, options: { value: string; label: string }[]) {
  if (!label || label === "Row count") return true;
  const fieldLabels = options.map((o) => o.label);
  return ["Count", "Sum", "Average", "Minimum", "Maximum"].some((op) =>
    fieldLabels.some((fl) => label === `${op} of ${fl}`)
  );
}
          ...
          <section className="builder-subsection">
            ...
            <div className="builder-subsection-grid">
              <label className="field">
                ...
                <SearchableSelect
                  ...
                  onChange={(value) => setCreateDraft((current) => {
                    ...
                    const currentLabel = metric?.label || "";
                    const useAutoLabel = !currentLabel || isAutoGeneratedLabel(currentLabel, fieldOptions);
                    return {
                      ...current,
                      groups: value ? [{ id: current.groups[0]?.id || `grp-1`, fieldId: value }] : [],
                      summaryMetrics: metric ? [{ ...metric, label: useAutoLabel ? (autoLabel || metric.label) : metric.label }] : current.summaryMetrics
                    };
                  })}
                />
              </label>
              <label className="field">
                ...
                <SearchableSelect
                  ...
                  onChange={(value) => setCreateDraft((current) => {
                    ...
                    const currentLabel = current.summaryMetrics[0]?.label || "";
                    const useAutoLabel = !currentLabel || isAutoGeneratedLabel(currentLabel, fieldOptions);
                    return {
                      ...current,
                      summaryMetrics: [{
                        ...
                        fieldId: value,
                        op,
                        label: useAutoLabel ? autoLabel : currentLabel
                      }]
                    };
                    void groupLabel;
                  })}
                  ...
                  onChange={(event) => setCreateDraft((current) => {
                    ...
                    return {
                      ...
                      summaryMetrics: current.summaryMetrics.length
                        ? [{ ...current.summaryMetrics[0], op, label: useAutoLabel ? autoLabel : currentLabel }]
              ...
              <label className="field">
                ...
                <input
                  type="text"
                  value={createDraft.summaryMetrics[0]?.label || ""}
                  placeholder="e.g. AGING DAYS →"
                  onChange={(event) => setCreateDraft((current) => ({
                    ...current,
                    summaryMetrics: current.summaryMetrics.length
                      ? [{ ...current.summaryMetrics[0], label: event.target.value }]
