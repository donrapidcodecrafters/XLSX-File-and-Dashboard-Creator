    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(12,22,18,0.58)", zIndex: 9999, display: "grid", placeItems: "center", padding: "24px" }}>
      <div style={{ width: "min(560px, 100%)", background: "#fff", borderRadius: "20px", padding: "24px", boxShadow: "0 24px 64px rgba(0,0,0,0.24)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.95rem", color: "#41554a" }}>
          <span>{terminal ? (status === "failed" ? "Refresh failed" : "Refresh cancelled") : indeterminate ? "Loading in progress" : `${progress}% complete`}</span>
          <span>{terminal ? "Review the message above" : indeterminate ? "Preparing results…" : (typeof job.estimatedSecondsRemaining === "number" ? `~${job.estimatedSecondsRemaining}s remaining` : "Estimating time…")}</span>
        </div>
        {terminal && onDismiss ? (
